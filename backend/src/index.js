...
async function start() {
  try {
    ...
    if (!admin) {
      console.log('[SEED] No data found, running seed...');
      const bcrypt = require('bcrypt');
      const hashed = await bcrypt.hash('admin123', 10);
      await prisma.user.create({
        data: { name: 'Store Owner', email: 'admin@store.com', password: hashed, role: 'OWNER' }
      });
      const vendors = await Promise.all([
        prisma.vendor.create({ data: { name: 'PharmaDistributors Ltd', contact: '9876543210', address: 'Mumbai', email: 'info@pharmadist.com' } }),
        prisma.vendor.create({ data: { name: 'MediSupply Co', contact: '9876543211', address: 'Delhi', email: 'orders@medisupply.co' } }),
        prisma.vendor.create({ data: { name: 'HealthCare Wholesale', contact: '9876543212', address: 'Bangalore', email: 'sales@healthcarewholesale.com' } }),
        prisma.vendor.create({ data: { name: 'City Chemist Distributors', contact: '9876543213', address: 'Pune' } }),
      ]);
      const products = [
      ...
      await prisma.settings.create({
        data: { storeName: 'Raman Medicos', storeAddress: '123, Main Road, City Center, India', storeGst: '22AAAAA0000A1Z5', storePhone: '9876543210', ownerName: 'Raman Sharma', expiryAlertDays: 90, defaultReorderLevel: 10 }
      });
      console.log('[SEED] Admin, vendors, products, and settings created');
    } else {
